...
// oblouku, protoze jsou body na sobe.
...
#ifndef BOOST_GEOMETRY_STRATEGIES_CARTESIAN_BUFFER_END_ROUND_FIX_HPP
#define BOOST_GEOMETRY_STRATEGIES_CARTESIAN_BUFFER_END_ROUND_FIX_HPP
...
private :
                ...
                RangeOut& range_out) const
    {
        ...
        {
            typename boost::range_value<RangeOut>::type p;
            set<0>(p, get<0>(point) + buffer_distance * cos(alpha2));
            set<1>(p, get<1>(point) + buffer_distance * sin(alpha2));
            range_out.push_back(p);
        }
    }
...
public :
    ...
    explicit inline end_round_fix(std::size_t points_per_circle = 90)
...
#endif // BOOST_GEOMETRY_STRATEGIES_CARTESIAN_BUFFER_END_ROUND_FIX_HPP
